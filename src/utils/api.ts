...
export const getInvoice = (id: string): Promise<Invoice & { items: InvoiceItem[] }> =>
  fetch(`${API.invoices}/${id}`).then(j);
export const getReceipt = (id: string): Promise<Receipt & { items: ReceiptItem[] }> =>
  fetch(`${API.receipts}/${id}`).then(j);
...
// ------- APPOINTMENTS (inkl. Monats-Fetch) -------
