...
// ---- Connection & SSL (Railway kompatibel) ---------------------------------
const connectionString = process.env.DATABASE_URL;
if (!connectionString) {
  throw new Error("DATABASE_URL fehlt in der Umgebung (.env)");
}
// Railway intern kein SSL nötig; extern: SSL erzwingen
const needsSSL = !/postgres\.railway\.internal/.test(connectionString);
...
// Nur Felder updaten, die erlaubt sind; setzt updatedAt automatisch
...
// Diese Funktionen stammen aus deiner bisherigen lib/db.js und sind hier konsolidiert,
// damit es KEIN doppeltes Schema-Setup mehr gibt. 

async function ensureCoreSchema(client: any) {
  // Kunden
  await client.query(`
...
// Termine/Appointment Schema (aus lib/db.js übernommen). 
async function ensureAppointmentSchema(client: any) {
...
let __schemaReadyPromise: Promise<void> | null = null;

/**
 * ensureSchemaOnce()
 * Ruft die Schema-Initialisierung exakt einmal pro Prozess auf.
 * Kann in API-Routen vor der ersten Query aufgerufen werden.
 */
