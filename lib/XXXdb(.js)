...
// 2) Einmalige Initialisierung – wird beim ersten Aufruf durchgeführt
let __schemaReadyPromise = null;

/**
 * ensureSchemaOnce()
 * Ruft die Schema-Initialisierung exakt einmal pro Prozess auf.
 * In jeder API-Route: vor der ersten Query `await ensureSchemaOnce()` ausführen.
 */
export async function ensureSchemaOnce() {
  if (__schemaReadyPromise) return __schemaReadyPromise;
  __schemaReadyPromise = (async () => {
    const client = await pool.connect(); // Hinweis: `pool` ist in deiner lib/db.js bereits definiert
    try {
