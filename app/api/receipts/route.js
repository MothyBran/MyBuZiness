...
export async function GET(request) {
  try {
    ...
    const rows = (await q(
      `SELECT *
       FROM "Receipt"
       ${qs ? `WHERE lower("receiptNo") LIKE $1` : ""}
...
export async function POST(request) {
  try {
    ...
    for (const it of norm) {
      await q(
        `INSERT INTO "ReceiptItem" ("id","receiptId","productId","name","quantity","unitPriceCents","extraBaseCents","lineTotalCents","createdAt","updatedAt")
