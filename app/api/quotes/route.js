...
export async function POST(request) {
  ...
  const format = s.quoteNumberFormat || "{YYYY}-Q{SEQ4}"; // optional: eigene Spalte
  // einfache Sequenz
  const seq = (await q(`SELECT COALESCE(MAX(NULLIF(regexp_replace("quoteNo",'\\D','','g'), '')::bigint),0)+1 AS seq FROM "Quote"`)).rows[0].seq;
  ...
  if (prodIds.length) {
    const prows = (await q(
      `SELECT "id"::text AS id, COALESCE("vatRate", 19) AS "vatRate"
       FROM "Product" WHERE "id"::text = ANY($1::text[])`,
      [prodIds]
    )).rows;
  ...
  await q(`INSERT INTO "Quote"("id","quoteNo","customerId","issueDate","validUntil","currency","netCents","taxCents","grossCents","status","createdAt","updatedAt")
    ...
    [id, quoteNo, b.customerId, b.issueDate || null, b.validUntil || null, b.currency || s.currency || "EUR", netCents, finalTaxCents, grossCents]
  );
  for (const it of prepared) {
    await q(`INSERT INTO "QuoteItem"("id","quoteId","productId","name","quantity","unitPriceCents","lineTotalCents","vatRate","createdAt","updatedAt")
