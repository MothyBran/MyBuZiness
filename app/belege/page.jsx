...
function ReceiptDetails({ row, currency }){
  ...
  return (
    <div style={{display:"grid",gap:12}}>
      ...
      <div style={{textAlign:"right",fontWeight:800}}>Gesamt: {fmt(row.grossCents, currency)}</div>
    </div>
  );
}

/* ===== Editor (Neu/Bearbeiten) – unverändert funktional ===== */
function ReceiptEditor({ title, products, currency, kleinunternehmer, initial, onClose, onSaved }){
  ...
  async function save(e){
    ...
    if(isEdit){ url = `/api/receipts/${initial.id}`; method = "PUT"; }
      ...
      <form onSubmit={save} style={{display:"grid",gap:12}}>
        <div style={{display:"grid",gap:12,gridTemplateColumns:"1fr 1fr"}}>
          ...
          <Field label="Rabatt gesamt">
            <input style={input} value={discount} onChange={e=>setDiscount(e.target.value)} inputMode="decimal" placeholder={`z. B. ${fromCents(1000)}`} />
          </Field>
        </div>
        ...
        {kleinunternehmer && (
          <div style={{ fontSize:12, color:"#6b7280" }}>
            Hinweis: Kleinunternehmer-Regelung (§19 UStG) aktiv – es wird keine Umsatzsteuer berechnet/ausgewiesen.
          </div>
        )}

        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div />
          <div style={{fontWeight:800}}>Gesamt: {fmt(gross, currency)}</div>
        </div>
