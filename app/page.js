    ...
    <>
      ...
      <div className="hero">
        <Section>
          ...
          <p className="subtle">
            Schlanke Kennzahlen und die neuesten Bewegungen. Erfassung erfolgt in den jeweiligen Modulen.
          </p>
        </Section>
      </div>
      ...
      <Section>
        <div className="grid-3">
          <StatCard
            ...
            foot={<Muted>Gesamt</Muted>}
          />
          <StatCard
            title="Umsatz (aktueller Monat)"
            value={loading ? "—" : fromCents(turnoverMonth, currency)}
            foot={<Muted>{fromCents(turnoverInvoicesMonth, currency)} Rechnungen · {fromCents(turnoverReceiptsMonth, currency)} Belege</Muted>}
          />
          <StatCard
            title="Offene Rechnungen"
            value={loading ? "—" : invoices.filter(i => i.status === "open").length}
            foot={<Muted>Status „offen“</Muted>}
          />
        </div>
      </Section>
      ...
      <Section>
        <div className="grid-2">
          <Panel title="Zuletzt erstellte Rechnungen">
            <Table head={["Nr.", "Kunde", "Datum", "Brutto"]}>
